...
def format_issue_row(issue):
    ...
    # labels = [label["name"] for label in issue.get("labels", [])]  # noqa: F841
    ...
    return (
        f"| [# {number}]({url}) | {status_icon} | {title} | "
        f"{executor_name} | - | {milestone_title} |"
    )
...
"""
    ...
    for i in only_issues:
        executor = i.get("assignee", {}).get("login", "-") if i.get("assignee") else "-"
